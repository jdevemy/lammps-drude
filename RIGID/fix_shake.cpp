...
void FixShake::find_clusters()
{
  ...
  //memory->create(partner_tag,nlocal,5,"shake:partner_tag");
  int max = build_partner_tag(npartner, &partner_tag);
...
int FixShake::build_partner_tag(int *npartner, tagint ***ptr_partner_tag){
  int nlocal = atom->nlocal;
  int *num_bond = atom->num_bond;
  tagint **bond_atom = atom->bond_atom;

  partner_tag_set = new std::set<tagint>[nlocal];
  std::vector<tagint> bond_list_vec;
  for (int i=0; i<nlocal; i++) {
    for (int k=0; k<num_bond[i]; k++) {
      bond_list_vec.push_back(atom->tag[i]);
      bond_list_vec.push_back(bond_atom[i][k]);
    }
  }
  fsptr = this;
  comm->ring(bond_list_vec.size(), sizeof(tagint),
             (char *) bond_list_vec.data(),
             4, ring_find_partner, NULL, 1);
  int max, max_loc = 0;
  for (int i=0; i<nlocal; i++){
    npartner[i] = partner_tag_set[i].size();
    if (npartner[i] > max_loc) max_loc = npartner[i];
  }
  MPI_Allreduce(&max_loc, &max, 1, MPI_INT, MPI_MAX, world);
  tagint **partner_tag;
  memory->create(partner_tag, nlocal, max, "shake:partner_tag");
  for (int i=0; i<nlocal; i++){
    ...
    for (it = partner_tag_set[i].begin(), j = 0; 
         it != partner_tag_set[i].end(); 
         it++, j++) {
      partner_tag[i][j] = *it;
      //std::cout << *it << " ";
    }
  ...
  delete [] partner_tag_set;
  *ptr_partner_tag = partner_tag;
  return max;
}

void FixShake::ring_find_partner(int size, char *cbuf){
  Atom *atom = fsptr->atom;
  std::set<tagint> *partner_tag_set = fsptr->partner_tag_set;
  int nlocal = atom->nlocal;
  ...
  while (it < last) {
    int j = atom->map(*it);
    if (j >= 0 && j < nlocal)
      partner_tag_set[j].insert(*(it+1));
    j = atom->map(*(it+1));
    if (j >= 0 && j < nlocal)
      partner_tag_set[j].insert(*it);
    it += 2;
  }
}
